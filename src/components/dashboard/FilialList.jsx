...
const FilialList = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await addFilial(filialId, filialText, userPhone);
        ...
        <ul>
          {filials.map((filial) => (
            <div className="filial-el" key={filial.filial._id}>
                  <p>Название филиала: {filial.filial.filialText} <br /> Номер телефона: {filial.filial.userPhone} <br /> ID филиала: {filial.filial.filialId}</p>
